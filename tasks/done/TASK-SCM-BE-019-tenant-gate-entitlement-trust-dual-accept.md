...
> **완료 (2026-05-31)**: impl PR #964 (squash `0eab72c4`). ADR-MONO-019 § 3.3 step 3 복제 2/N (scm, 다중 서비스; opus dispatch). **4 enforcement 지점**(gateway `TenantClaimValidator` + procurement `TenantClaimValidator`/`TenantClaimEnforcer` + inventory-visibility `TenantClaimEnforcer`)을 `tenant_id==scm` 고정 → **entitlement-trust dual-accept**: legacy(`tenant_id ∈ {scm,*}`) ∪ 서명 `entitled_domains ∋ scm`, **거부=!legacyOk && !entitled**(fail-closed). **서비스별 로컬 isEntitled**(모듈 공유 불가; procurement 내부 validator↔enforcer 공유). `TenantClaimExtractor` 무변경(게이트 아님). **net-zero**(claim 부재 시 legacy만). 격리 IT entitled→통과 / non-entitled→403. 3 architecture.md(gateway/procurement/inventory-visibility) 갱신. **3차원**(MERGED `0eab72c4` / tip 일치 / pre-merge 0 failing). **BE-299 re-stage** ✓. **CI 1-pass**: scm Integration GREEN 1m56s + scm e2e GREEN. **scope-lock**: scm 3서비스만. **남은 step 3**: wms(6-svc)/gap+console-bff + GAP `entitled_domains` populate(shared). **메타**: 다중 서비스 도메인은 모듈 경계로 isEntitled 공유 불가 → 서비스별 로컬 헬퍼(서비스 내 validator+enforcer만 공유).
